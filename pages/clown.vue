...
.caught-screen { align-items: center; background: rgba(25, 0, 12, .27); color: #fff; display: flex; flex-direction: column; inset: 0; justify-content: center; position: fixed; text-align: center; text-shadow: 3px 3px 0 #421126; z-index: 21; }.scary-clown { animation: scary-pop .45s cubic-bezier(.18, .8, .2, 1) both, clown-wiggle .35s .45s ease-in-out infinite alternate; filter: contrast(1.35) saturate(1.65) drop-shadow(0 0 22px #f31b53); font-size: clamp(9rem, 42vw, 22rem); line-height: .8; }.caught-screen p { font-size: clamp(2rem, 12vw, 4.7rem); margin: 12px 0 4px; }.caught-screen .run-button { margin-top: 16px; position: relative; z-index: 22; }
@keyframes puff { from { opacity: 0; transform: scale(.5); } to { opacity: 1; transform: translateX(18px) scale(1.2); } } @keyframes pop { from { opacity: 0; transform: translateX(-50%) scale(.4); } to { opacity: 1; transform: translateX(-50%) scale(1); } } @keyframes clown-wiggle { to { transform: rotate(6deg); } } @keyframes runner-jump { to { transform: translateY(-12px) scaleX(-1); } } @keyframes button-bounce { 50% { transform: translateY(-3px); } } @keyframes choice-pulse { to { box-shadow: 0 5px 0 #58435a, 0 0 0 7px rgba(255, 226, 92, .58); transform: scale(1.04); } } @keyframes break-away { from { opacity: 0; transform: translate(0) rotate(0); } to { opacity: 1; transform: translate(var(--x), var(--y)) rotate(var(--rotate)); } } @keyframes crack-grow { from { opacity: 0; transform: rotate(var(--angle)) scaleX(0); } to { opacity: 1; transform: rotate(var(--angle)) scaleX(1); } } @keyframes impact { from { opacity: 0; transform: scale(3); } to { opacity: 1; transform: scale(1); } } @keyframes scary-pop { from { opacity: 0; transform: scale(.2) rotate(-12deg); } to { opacity: 1; transform: scale(1) rotate(0); } }
