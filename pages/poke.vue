<template>
  <div class="card-wrapper" @touchmove.prevent>
...
export default {
  ...
  mounted() {
    ...
    // 初期状態で効果を適用したい場合
    setTimeout(() => {
      const initialX = CARD.getBoundingClientRect().x + 200;  // 任意の初期位置
      const initialY = CARD.getBoundingClientRect().y + 400;  // 任意の初期位置
...
.card-wrapper {
  ...
  box-shadow: 0 10px 20px -5px #515151;
  touch-action: none;
}
.card-wrapper.hover {
