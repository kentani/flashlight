<template>
  <div class="word-game" :class="gameClass">
    <GameStartOverlay v-if="!isReady" title="もじあそび" message="おなじ もじを みつけよう！" @start="startGame" />
...
<style scoped>
.word-game { min-height: 100vh; font-family: "Yomogi", cursive; overflow: hidden; background: linear-gradient(#c9f3ff 0%, #f3fdff 68%, #b6ecf7 100%); }
...
.bubble { position: absolute; z-index: 0; border: 3px solid rgba(255, 255, 255, .66); border-radius: 50%; background: rgba(114, 209, 235, .18); pointer-events: none; animation: float 3.2s ease-in-out infinite alternate; }.bubble--one { width: 54px; height: 54px; top: 18%; left: -17px; }.bubble--two { width: 27px; height: 27px; top: 54%; right: 4%; animation-delay: .8s; }.bubble--three { width: 38px; height: 38px; top: 76%; left: 6%; animation-delay: 1.4s; }
.game-rule, .section1, .section2, .section3 { position: relative; z-index: 1; }
.game-rule { display: flex; justify-content: center; gap: 9px; max-width: 680px; margin: 0 auto 12px; color: #176080; font-weight: bold; font-size: clamp(14px, 3.8vw, 19px); }
.game-rule span { background: rgba(255, 255, 255, .82); border: 2px solid #8bdbed; border-radius: 999px; box-shadow: 0 3px 0 rgba(32, 139, 172, .16); padding: 7px 10px; white-space: nowrap; }
.section1 { display: grid; grid-template-columns: minmax(0, 1.15fr) minmax(104px, .85fr); gap: 12px; max-width: 760px; margin: auto; align-items: center; }
...
.section1 .card { position: relative; display: flex; align-items: center; justify-content: center; border: 4px solid #52c9e6; box-shadow: 0 8px 0 rgba(29, 133, 167, .2); background: linear-gradient(135deg, #fff 0%, #e5fbff 100%); }.theme-hint { position: absolute; top: 11px; left: 50%; width: max-content; max-width: calc(100% - 16px); padding: 3px 11px; border-radius: 999px; background: #168fbd; color: #fff; font-size: clamp(13px, 3vw, 18px); font-weight: bold; transform: translateX(-50%) rotate(-2deg); }.section1 .card-title { color: #1b9bc5; line-height: 1; text-shadow: 0 4px 0 #bcecf6; }.section1 .card-title span { display: block; font-size: clamp(112px, 25vw, 180px); font-weight: bold; line-height: .9; }
.c-stepper { display: flex; align-items: center; justify-content: center; }.c-stepper__item { text-align: center; }.c-stepper__title { margin: 0; }.c-stepper__title > span { display: grid; place-items: center; color: #176080; font-weight: bold; font-size: clamp(48px, 13vw, 80px); width: clamp(104px, 28vw, 230px); height: clamp(104px, 28vw, 220px); border: 6px solid #fff; border-radius: 50%; box-shadow: 0 7px 0 rgba(25, 125, 158, .2); }
.section2 { max-width: 760px; margin: auto; }.card-list { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }.section2 .card { display: grid; min-height: clamp(78px, 18vw, 116px); place-items: center; border: 4px solid rgba(255, 255, 255, .92); box-shadow: 0 6px 0 rgba(26, 104, 132, .22); color: #174d68; text-align: center; font-size: clamp(42px, 12vw, 82px); line-height: 1; font-weight: bold; cursor: pointer; transition: transform .15s, box-shadow .15s, filter .15s; }.section2 .card:not(:disabled):hover { filter: brightness(1.06); transform: translateY(-2px) rotate(-1deg); }.section2 .card:not(:disabled):active { box-shadow: 0 2px 0 rgba(26, 104, 132, .22); transform: translateY(4px) scale(.96); }.section2 .card:disabled { cursor: default; }.section2 .card.selected { box-shadow: 0 2px 18px rgba(8, 89, 116, .48); }.section2 .card.a { background: #a9ebf7; }.section2 .card.b { background: #b9f0d7; }.section2 .card.c { background: #d3e9ff; }.section2 .card.d { background: #d5c8ff; }.section2 .card.sm, .section2 .card.md, .section2 .card.lg, .section2 .card.xl { font-size: clamp(42px, 12vw, 82px); }.section2 .card.a, .section2 .card.b, .section2 .card.c, .section2 .card.d { width: auto; }
.section3 { max-width: 760px; margin: 9px auto; text-align: center; }.section3 p { display: inline-block; margin: 0; padding: 4px 12px; border-radius: 999px; background: rgba(255, 255, 255, .56); color: #176080; font-weight: bold; font-size: clamp(19px, 5.2vw, 27px); letter-spacing: 1px; }.card.result { border: 5px solid #f5b648; animation: correct-pop .35s ease-out; }
.result-screen { position: fixed; z-index: 5; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #fff; background: rgba(5, 10, 19, .78); font-family: "Yomogi", cursive; text-align: center; animation: appear .35s ease-out; }.result-screen--failed { background: rgba(5, 10, 19, .1); text-shadow: 2px 3px 0 #16212c; }.result-screen__title { font-size: clamp(50px, 15vw, 110px); font-weight: bold; margin: 0; }.result-screen p:not(.result-screen__title) { font-size: 24px; }.retry-button { display: block; margin-top: 12px; padding: 14px 32px; border: 4px solid #fff; border-radius: 999px; background: #26c6da; color: #fff; font: bold 24px "Yomogi", cursive; cursor: pointer; }
