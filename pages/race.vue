<template>
  <div class="game-page" :class="{ crash: result === 'crash' }">
    <GameScreenLayout class="race" aria-label="くるまレース">
      <template #heading><div><h1>くるまレース！</h1><p>{{ guide }}</p></div></template>
      <template #progress><GameProgressPanel :items="progressItems" /></template>
      <template #board>
        <div class="track"><span class="road"></span><span v-for="x in items" :key="x.id" :class="['item','lane'+x.lane]" :style="{ left: x.x + '%' }">{{ x.type === 'gas' ? '⛽' : '🚧' }}</span><span :class="['car','lane'+lane]">🏎️</span><b v-if="message" class="pop">{{ message }}</b></div>
      </template>
      <template #actions>
        <div class="buttons" :class="{ 'is-ready': !playing, 'is-playing': playing }"><button v-if="playing" @click="up">⬆️ うえ</button><GameActionButtons v-else-if="!result" @primary="start">はじめる</GameActionButtons><button v-if="playing" @click="down">した ⬇️</button></div>
      </template>
    </GameScreenLayout>
    <GameResultOverlay v-if="result" :title="result === 'win' ? 'ゴール！' : 'もういちど！'" :celebration="result === 'win' ? '🏎️🏁🎉' : '🏎️💪'" :tone="result === 'win' ? 'clear' : 'retry'" @retry="start">{{ result === 'win' ? 'さいごまで はしれたね！' : 'こんどは よけて すすもう！' }}</GameResultOverlay>
  </div>
</template>
<script>
